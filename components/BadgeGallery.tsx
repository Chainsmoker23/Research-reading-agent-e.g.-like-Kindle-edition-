    ...
    <div className="flex-1 overflow-y-auto bg-main p-6 pb-20 animate-fade-in">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16 pt-8">
          ...
          <h1 className="font-serif text-4xl md:text-5xl font-bold text-textMain mb-4">
            Hall of Wisdom
          </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-16">
          {RANKS.map((rank, index) => {
            ...
            // Determine special styling for top tiers
            ...
            return (
              <div 
                ...
                className={`
                  relative p-4 rounded-2xl border transition-all duration-300 flex flex-col items-center text-center cursor-pointer group
                  ${isUnlocked 
                    ? 'bg-surface border-borderSkin shadow-md hover:shadow-xl hover:-translate-y-1' 
                    : 'bg-surface/50 border-borderSkin/50 opacity-70 hover:opacity-100 grayscale hover:grayscale-0'
                  }
                  ${isGodTier && isUnlocked ? 'border-amber-300 shadow-amber-100/50' : ''}
                `}
              >
                {/* Badge Visual */}
                <div className="mb-4 mt-2 transition-transform duration-500 group-hover:scale-110 relative">
                ...
                {/* Title */}
                <h4 className={`font-serif font-bold text-sm mb-2 leading-tight ${isUnlocked ? 'text-textMain' : 'text-textMuted'}`}>
                  {rank.title}
                </h4>
                
                {/* Subtext */}
                <div className="flex flex-col items-center gap-1 text-xs w-full mt-auto pt-2">
                  <span className={`px-2 py-0.5 rounded-full font-mono ${isUnlocked ? 'bg-main text-textMuted' : 'bg-transparent text-textMuted/70'}`}>
                ...
                {/* Hover Reveal Tip */}
                <div className="absolute inset-0 bg-black/80 rounded-2xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity p-4 text-white backdrop-blur-sm z-20 pointer-events-none">
                  <p className="text-xs font-medium">Click to view {isUnlocked ? 'stats' : 'requirements'}</p>
                </div>
              </div>
            );
          })}
        ...
        {/* Legendary Footer */}
        <div className="text-center bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-amber-500/10 p-8 rounded-3xl border border-amber-200/50">
           <div className="flex items-center justify-center gap-2 mb-2 text-amber-700 font-bold uppercase tracking-widest text-sm">
             <Crown size={16} />
             Ultimate Goal
           </div>
           <p className="font-serif text-xl italic text-textMain max-w-xl mx-auto">
